...
export function GlassmorphismTemplate({ data }: GlassmorphismTemplateProps) {
  ...
  const templateSettings = (data.templateSettings || {}) as {
  ...
  // Calculate or use custom values for stats
  const calculatedYears = experience.length > 0
    ? new Date().getFullYear() - new Date(experience[experience.length - 1]?.startDate || new Date()).getFullYear()
    : 0;
  const yearsOfExperience = templateSettings.yearsExperience ?? calculatedYears;
  const skillsCount = templateSettings.skillsCount ?? skills.length;
  const projectsCount = templateSettings.projectsCount ?? projects.length;
  const showStatsWidgets = templateSettings.showStatsWidgets !== false; // Default true
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="order-1 lg:order-2 flex justify-center">
                <div className="relative">
                  ...
                  {/* Floating Stats Widgets - Can be toggled via template settings */}
                  {showStatsWidgets && (
                    <>
